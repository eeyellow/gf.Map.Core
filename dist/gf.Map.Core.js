...
function GEEMap(_map) {
  var o = this;
  var map = _map; // Initialize the layers data.
  ...
  var locateDataPool = []; // from fusion_extended_map.js
  ...
  function geeMapTileFunc(request, server, glmId, channel, version) {
    return function (coord, zoom) {
      ...
      url += '&channel=' + channel + '&version=' + version + '&x=' + x + '&y=' + coord.y + '&z=' + zoom;
      return url;
    };
  }

  function geeMapImageryFunc(requestType, server, glmId, channel, version) {
    return geeMapTileFunc(requestType, server, glmId, channel, version);
  }

  function geeMapVectorFunc(server, glmId, channel, version) {
    return geeMapTileFunc('VectorMapsRaster', server, glmId, channel, version);
  }
  ...
  map.addGEE = function (imgName, geeId, url, storage) {
    ...
    var defKey = "geeServerDefs" + "".concat(d);
  ...
  function updateMapType(map, method, mt_id) {
    if (!map) return false;
    var maptypes = map.mapTypeControlOptions.mapTypeIds; //unshift：放到最前

    if (method == 'unshift') maptypes.unshift(mt_id); //push：放到最後
  ...
  map.initializeLayers = function (serverDefs, imgName, geeId) {
    ...
    for (var i = 0; i < layerDefs.length; ++i) {
      ...
      var channel = layerDefs[i].id; // Use layer glm_id if it is defined. Otherwise, set it to 0.
      ...
      if (layerDefs[i].requestType == 'VectorMapsRaster') {
        this.addLayer(name, numLayers, map, isPng, geeMapVectorFunc(serverUrl, glmId, channel, version), glmId, channel, enabled);
      } else if (geeIsImageryLayer(layerDefs[i])) {
        this.addLayer(name, numLayers, map, isPng, geeMapImageryFunc(requestType, serverUrl, glmId, channel, version), glmId, channel, enabled);
      } // geeId
  ...
  map.addLayer = function (name, index, map, isPng, urlFunction, glmId, channel, enabled) {
    var options = {
    ...
    options.tileSize = new google.maps.Size(options.tileWidth, options.tileHeight);
    var overlay = new google.maps.ImageMapType(options);

    if (enabled) {} // don't enable by default
    ...
    this.options[id] = options; // img overlay add to maptype
  ...
  map.showFusionLayer = function (id) {
    if (typeof this.layerMap[id] != 'undefined') {
      this.overlayMapTypes.setAt(this.layerMap[id].index, this.layerMap[id].overlay);
      this.layerVisible[id] = true;
    } else {//  alert('Unknown layer: ' + id);
    }
  };
  /**
  ...
  map.addWMSLayer = function (id, wmsurl) {
    var wmsLayer = new google.maps.ImageMapType({
      getTileUrl: function getTileUrl(coord, zoom) {
        ...
        var bot = proj.fromPointToLatLng(new google.maps.Point((coord.x + 1) * 256 / zfactor, (coord.y + 1) * 256 / zfactor)); //corrections for the slight shift of the SLP (mapserver)
        //var deltaX = 0.0013;
        //var deltaY = 0.00058;
        ...
        switch (version) {
          case "1.1.0":
          case "1.1.1":
            newtop = proj4("EPSG:3857", "EPSG:3857", {
              x: fixtoplng,
              y: fixtoplat
            });
            newbot = proj4("EPSG:3857", "EPSG:3857", {
              x: fixbotlng,
              y: fixbotlat
            });
            break;
          ...
          case "1.3.0":
            newtop = proj4("EPSG:4326", "EPSG:3857", {
              x: fixtoplng,
              y: fixtoplat
            });
            newbot = proj4("EPSG:4326", "EPSG:3857", {
              x: fixbotlng,
              y: fixbotlat
            });
            break;
        ...
        var bbox = newtop.x + "," + newbot.y + "," + newbot.x + "," + newtop.y; //base WMS URL
  ...
  map.removeWMSLayer = function (id) {
    this.wmsLayer.forEach(function (ele, idx) {
      if (ele == id) {
        ...
        if (o && o.id == id) {
          map.overlayMapTypes.removeAt(i); //map.overlayMapTypes.setAt(i, null);
        }
      }
    });
  ...
  map.setGeoJsonLayer = function (id, dataLayer) {
    if (id != undefined) {
      dataLayer.setMap(map); //this.geojsonLayer[id] = dataLayer;
  ...
  map.listSPOT = function () {
    var objSPOT = function objSPOT(location, year, type) {
      var self = this;
    ...
    var yearStart = 1996;
    var yearEnd = 2017;
    var types = ["自然色", "假色"]; //var locations = ["臺灣", "澎湖"];
  ...
  map.toggleSPOT = function (toggle, opacity, year, type) {
    if (toggle == true) {
      map.overlayMapTypes.getArray().filter(function (ele) {
        return ele != undefined && ele.name && ele.name.substring(0, 4) == "SPOT";
      }).forEach(function (ele) {
        ele.setOpacity(0);
      });
      var idx = map.overlayMapTypes.getArray().findIndex(function (ele) {
      ...
      if (idx > -1) {
        ...
        var mapMinZoom = 6;
        var mapMaxZoom = 18;
        var mapBounds = new google.maps.LatLngBounds(new google.maps.LatLng(21.81785406, 119.86499422), new google.maps.LatLng(25.4334898, 122.05204106));
        var entry;
